...
def _load_workflow_sync(workflow_id: str):
    ...
    try:
        cur = conn.cursor()
        cur.execute("SELECT id, name, description, trigger_type, trigger_config FROM workflows WHERE id=?", (workflow_id,))
        row = cur.fetchone()
        if not row:
            return None, [], [], {}
        workflow = {"id": row[0], "name": row[1], "description": row[2], "trigger_type": row[3]}

        cur.execute("SELECT id, workflow_id, agent_id, node_type, label, position_x, position_y, config FROM workflow_nodes WHERE workflow_id=?", (workflow_id,))
        nodes = [{"id": r[0], "workflow_id": r[1], "agent_id": r[2], "node_type": r[3], "label": r[4], "position_x": r[5], "position_y": r[6], "config": r[7]} for r in cur.fetchall()]

        cur.execute("SELECT id, workflow_id, source_node_id, target_node_id, condition, label FROM workflow_edges WHERE workflow_id=?", (workflow_id,))
        edges = [{"id": r[0], "workflow_id": r[1], "source_node_id": r[2], "target_node_id": r[3], "condition": r[4], "label": r[5]} for r in cur.fetchall()]

        agent_ids = [n["agent_id"] for n in nodes if n.get("agent_id")]
        agent_map = {}
        for aid in agent_ids:
            cur.execute("SELECT id, name, role, system_prompt, model, provider, temperature, max_iterations, memory_enabled, tools, max_output_tokens, guardrail_keywords FROM agents WHERE id=?", (aid,))
            ar = cur.fetchone()
            if ar:
                import json
                agent_map[ar[0]] = {
...
def _update_execution_sync(
    execution_id: str, status: str, output: str = None, error: str = None,
    celery_task_id: str = None, input_tokens: int = 0, output_tokens: int = 0, cost_usd: float = 0.0,
):
    conn = _get_sync_db()
    try:
        cur = conn.cursor()
        ...
        if status == "running":
            ...
            cur.execute("UPDATE executions SET status=? WHERE id=?", (status, execution_id))
...
def _save_log_sync(execution_id: str, level: str, message: str, agent_id: str = None, agent_name: str = None):
    import uuid
    conn = _get_sync_db()
    try:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO execution_logs (id, execution_id, agent_id, agent_name, level, message, metadata, timestamp) VALUES (?,?,?,?,?,?,?,?)",
            (str(uuid.uuid4()), execution_id, agent_id, agent_name, level, message, "{}", datetime.utcnow().isoformat())
        )
        conn.commit()
    finally:
        conn.close()
...
def run_workflow_task(self, workflow_id: str, execution_id: str, input_data: dict):
    try:
        ...
        log_emitter.emit(execution_id, "info", f"Starting workflow execution", metadata={"workflow_id": workflow_id})
        ...
        log_emitter.emit(execution_id, "info", f"Loaded {len(agent_nodes)} agent(s): {', '.join(agent_map[n['agent_id']]['name'] for n in agent_nodes if n['agent_id'] in agent_map)}")
        ...
        # Accumulate token usage emitted by log events (parsed from Redis-persisted logs)
        total_in, total_out, total_cost = _sum_token_usage_from_logs(execution_id)

        _save_log_sync(execution_id, "info",
        ...
        _update_execution_sync(execution_id, "completed", output=output,
...
def _sum_token_usage_from_logs(execution_id: str) -> tuple[int, int, float]:
    """Read persisted execution logs and sum up token usage from llm_end entries."""
    import json as _json
    conn = _get_sync_db()
    try:
        cur = conn.cursor()
        cur.execute("SELECT metadata FROM execution_logs WHERE execution_id=? AND level='llm_end'", (execution_id,))
